-- ============================================================
-- Phase 5B-2H customer_number NULL化 apply（本番適用・COMMIT前提）
--
-- 目的：
-- 他店舗匿名化閲覧RPCのうち、customer_numberの実値を返している次の
-- 2関数について、RETURN QUERY内のcustomer_number出力値だけを
-- 「c.customer_number」から「null::text」へ置き換える。戻り値の列名・
-- 列順・列型は一切変更しないため DROP FUNCTION は使用せず、
-- CREATE OR REPLACE FUNCTION だけで適用する。
--
...
--
-- 背景：
-- Phase 5B-2H本番読み取り専用監査（supabase_phase5b_2h_other_store_rpc_
-- readonly_audit.sql）で対象4関数の本番状態を実測し、customer_numberを
-- 返しているのは上記2関数だけであることを確認した。続けて実施した
-- Phase 5B-2H preflight（supabase_phase5b_2h_customer_number_null_
-- preflight.sql、本番で実行し必ずROLLBACKする検証専用トランザクション）が
-- 本番で正常に完了し、対象2関数のCREATE OR REPLACE FUNCTIONが
-- PostgreSQLに受理されること、およびpg_get_functiondef()による
-- 変更後の正規化定義MD5を実測済みである。本ファイルの2つの
-- CREATE OR REPLACE FUNCTION文は、そのpreflight内の定義を一字一句
-- そのまま再利用する（整形・コメント変更・空白調整は行わない）。
--
-- 変更前・変更後・変更対象外関数の正規化定義MD5（本番PostgreSQL上で
-- 下記の正規化式を直接実行して確定した実測値。外部ツールでの再計算値
-- ではない）：
...
-- 「c.customer_number」を「null::text」へ置き換えた1箇所だけ。関数名・
-- 入力引数・RETURNS TABLEの列数/列名/列順/列型・owner=postgres・
-- language=plpgsql・STABLE・SECURITY DEFINER・SET search_path = ''・
-- strict=false・leakproof=false・parallel=UNSAFE・auth.uid()を起点とする
-- 認証確認・role='admin'の確認・store_idの確認・is_super_admin分岐・
-- 自店舗と他店舗の判定・存在しない対象をpermission deniedとして扱う
-- 処理・SQLSTATE 42501・FROM/JOIN/WHERE/ORDER BY・体重/食事記録/食事
-- 写真有無の取得処理は、リポジトリ確定済み定義
-- （supabase_other_store_detail_customer_number_rpc_update.sql /
--  supabase_other_store_list_summary_rpc_update.sql）およびpreflightで
-- MD5一致確認済みの本番定義から一切変更しない。
--
-- GRANT・REVOKEは使用しない。CREATE OR REPLACE FUNCTIONは既存関数の
-- OID・所有者・権限（ACL）を変更しないというPostgreSQLの仕様により、
-- 適用前後でOID・owner・ACLが維持されることをprecondition/postcondition
-- の両方で実測確認する。
--
-- precondition（最初のCREATE OR REPLACE FUNCTIONより前）が1項目でも
-- 不一致ならRAISE EXCEPTIONで停止し、関数を一切変更しない。
-- postcondition（CREATE OR REPLACE FUNCTION後、COMMITより前）が1項目でも
-- 不一致ならRAISE EXCEPTIONで停止し、トランザクション全体を確定しない
-- （呼び出し側でROLLBACKが必要になる。この場合、本番の関数定義は
-- postconditionが実行された時点の内容のまま未確定であり、明示的な
-- ROLLBACKまたは接続断で破棄される）。
--
...
declare
  ...
  -- 対象外2関数（sibling）：apply前後で不変であることを確認するための現在値
...
begin
  ...
  if v_func1_oid::oid <> v_func1_expected_oid then
    raise exception 'precondition failed [F1-OID]: admin_get_other_store_client OID is % but expected % (production object identity has drifted since the Phase 5B-2H audit)', v_func1_oid::oid, v_func1_expected_oid;
  end if;
  ...
  if v_func1_identity is distinct from 'p_client_id uuid' then
    raise exception 'precondition failed [F1-04]: admin_get_other_store_client identity_arguments is not "p_client_id uuid" (found %)', v_func1_identity;
  end if;
  if v_func1_prokind <> 'f' then
    raise exception 'precondition failed [F1-05]: admin_get_other_store_client prokind is not a plain function (found %)', v_func1_prokind;
  end if;
  if v_func1_owner is distinct from 'postgres' then
    raise exception 'precondition failed [F1-06]: admin_get_other_store_client owner is not "postgres" (found %)', v_func1_owner;
  end if;
  if v_func1_lang is distinct from 'plpgsql' then
    raise exception 'precondition failed [F1-07]: admin_get_other_store_client language is not "plpgsql" (found %)', v_func1_lang;
  end if;
  if v_func1_secdef is distinct from true then
    raise exception 'precondition failed [F1-08]: admin_get_other_store_client is not SECURITY DEFINER';
  end if;
  if v_func1_volatile is distinct from 's' then
    raise exception 'precondition failed [F1-09]: admin_get_other_store_client volatility is not STABLE (found %)', v_func1_volatile;
  end if;
  if v_func1_strict is distinct from false then
    raise exception 'precondition failed [F1-10]: admin_get_other_store_client strict is not false';
  end if;
  if v_func1_leakproof is distinct from false then
    raise exception 'precondition failed [F1-11]: admin_get_other_store_client leakproof is not false';
  end if;
  if v_func1_parallel is distinct from 'u' then
    raise exception 'precondition failed [F1-12]: admin_get_other_store_client parallel is not UNSAFE (found %)', v_func1_parallel;
  end if;
  if v_func1_proconfig is null or array_length(v_func1_proconfig, 1) <> 1 or v_func1_proconfig[1] <> 'search_path=""' then
    raise exception 'precondition failed [F1-13]: admin_get_other_store_client proconfig is not exactly array[search_path=""] (found %)', v_func1_proconfig;
  end if;
  ...
  if v_func1_cols_name is distinct from v_expected_cols1_name or v_func1_cols_type is distinct from v_expected_cols1_type then
    raise exception 'precondition failed [F1-14]: admin_get_other_store_client output columns do not match the Phase 5B-2H audit result. names=% types=%', v_func1_cols_name, v_func1_cols_type;
  end if;

  v_func1_cn_pos  := array_position(v_func1_cols_name, 'customer_number');
  v_func1_cn_type := v_func1_cols_type[v_func1_cn_pos];
  if v_func1_cn_pos is null or (select count(*) from unnest(v_func1_cols_name) x where x = 'customer_number') <> 1 then
    raise exception 'precondition failed [F1-15]: admin_get_other_store_client does not have exactly one customer_number output column';
  end if;
  if v_func1_cn_type is distinct from 'text' then
    raise exception 'precondition failed [F1-16]: admin_get_other_store_client customer_number column type is not text (found %)', v_func1_cn_type;
  end if;
  ...
  -- ACLの意味的確認：EXECUTEを持つのはpostgres・authenticated・service_roleの
  -- 3ロールだけであること（それ以外のロール・それ以外の権限種別が1件でも
  -- あれば失敗させる）。
  ...
  if v_func1_acl_bad_count <> 0 then
    raise exception 'precondition failed [F1-21a]: admin_get_other_store_client has % ACL entr(y/ies) outside {postgres,authenticated,service_role}:EXECUTE', v_func1_acl_bad_count;
  end if;
  ...
  if v_func1_acl_good_count <> 3 then
    raise exception 'precondition failed [F1-21b]: admin_get_other_store_client does not have exactly 3 EXECUTE ACL entries for {postgres,authenticated,service_role} (found %)', v_func1_acl_good_count;
  end if;
  ...
  if not v_func1_owner_bypassrls then
    raise exception 'precondition failed [F1-22]: owner % of admin_get_other_store_client does not have rolbypassrls=true', v_func1_owner;
  end if;
  ...
  if v_func1_dep_count <> 0 then
    raise exception 'precondition failed [F1-23]: admin_get_other_store_client unexpectedly has % dependent catalog object(s) (expected 0)', v_func1_dep_count;
  end if;
  ...
  if v_func1_def_md5 is distinct from v_func1_md5_before then
    raise exception 'precondition failed [F1-MD5]: admin_get_other_store_client normalized definition MD5 is % but expected % (production definition has drifted since the Phase 5B-2H preflight)', v_func1_def_md5, v_func1_md5_before;
  end if;
  ...
  perform set_config('phase5b2h_apply.f1_pre_oid',   v_func1_oid::oid::text, true);
  perform set_config('phase5b2h_apply.f1_pre_owner', v_func1_owner, true);
  perform set_config('phase5b2h_apply.f1_pre_acl',   v_func1_pre_acl::text, true);
  ...
  if v_func2_oid::oid <> v_func2_expected_oid then
    raise exception 'precondition failed [F2-OID]: admin_list_other_store_clients OID is % but expected % (production object identity has drifted since the Phase 5B-2H audit)', v_func2_oid::oid, v_func2_expected_oid;
  end if;
  ...
  if v_func2_identity is distinct from 'p_store_id uuid' then
    raise exception 'precondition failed [F2-04]: admin_list_other_store_clients identity_arguments is not "p_store_id uuid" (found %)', v_func2_identity;
  end if;
  if v_func2_prokind <> 'f' then
    raise exception 'precondition failed [F2-05]: admin_list_other_store_clients prokind is not a plain function (found %)', v_func2_prokind;
  end if;
  if v_func2_owner is distinct from 'postgres' then
    raise exception 'precondition failed [F2-06]: admin_list_other_store_clients owner is not "postgres" (found %)', v_func2_owner;
  end if;
  if v_func2_lang is distinct from 'plpgsql' then
    raise exception 'precondition failed [F2-07]: admin_list_other_store_clients language is not "plpgsql" (found %)', v_func2_lang;
  end if;
  if v_func2_secdef is distinct from true then
    raise exception 'precondition failed [F2-08]: admin_list_other_store_clients is not SECURITY DEFINER';
  end if;
  if v_func2_volatile is distinct from 's' then
    raise exception 'precondition failed [F2-09]: admin_list_other_store_clients volatility is not STABLE (found %)', v_func2_volatile;
  end if;
  if v_func2_strict is distinct from false then
    raise exception 'precondition failed [F2-10]: admin_list_other_store_clients strict is not false';
  end if;
  if v_func2_leakproof is distinct from false then
    raise exception 'precondition failed [F2-11]: admin_list_other_store_clients leakproof is not false';
  end if;
  if v_func2_parallel is distinct from 'u' then
    raise exception 'precondition failed [F2-12]: admin_list_other_store_clients parallel is not UNSAFE (found %)', v_func2_parallel;
  end if;
  if v_func2_proconfig is null or array_length(v_func2_proconfig, 1) <> 1 or v_func2_proconfig[1] <> 'search_path=""' then
    raise exception 'precondition failed [F2-13]: admin_list_other_store_clients proconfig is not exactly array[search_path=""] (found %)', v_func2_proconfig;
  end if;
  ...
  if v_func2_cols_name is distinct from v_expected_cols2_name or v_func2_cols_type is distinct from v_expected_cols2_type then
    raise exception 'precondition failed [F2-14]: admin_list_other_store_clients output columns do not match the Phase 5B-2H audit result. names=% types=%', v_func2_cols_name, v_func2_cols_type;
  end if;

  v_func2_cn_pos  := array_position(v_func2_cols_name, 'customer_number');
  v_func2_cn_type := v_func2_cols_type[v_func2_cn_pos];
  if v_func2_cn_pos is null or (select count(*) from unnest(v_func2_cols_name) x where x = 'customer_number') <> 1 then
    raise exception 'precondition failed [F2-15]: admin_list_other_store_clients does not have exactly one customer_number output column';
  end if;
  if v_func2_cn_type is distinct from 'text' then
    raise exception 'precondition failed [F2-16]: admin_list_other_store_clients customer_number column type is not text (found %)', v_func2_cn_type;
  end if;
  ...
  if v_func2_acl_bad_count <> 0 then
    raise exception 'precondition failed [F2-21a]: admin_list_other_store_clients has % ACL entr(y/ies) outside {postgres,authenticated,service_role}:EXECUTE', v_func2_acl_bad_count;
  end if;
  ...
  if v_func2_acl_good_count <> 3 then
    raise exception 'precondition failed [F2-21b]: admin_list_other_store_clients does not have exactly 3 EXECUTE ACL entries for {postgres,authenticated,service_role} (found %)', v_func2_acl_good_count;
  end if;
  ...
  if not v_func2_owner_bypassrls then
    raise exception 'precondition failed [F2-22]: owner % of admin_list_other_store_clients does not have rolbypassrls=true', v_func2_owner;
  end if;
  ...
  if v_func2_dep_count <> 0 then
    raise exception 'precondition failed [F2-23]: admin_list_other_store_clients unexpectedly has % dependent catalog object(s) (expected 0)', v_func2_dep_count;
  end if;
  ...
  if v_func2_def_md5 is distinct from v_func2_md5_before then
    raise exception 'precondition failed [F2-MD5]: admin_list_other_store_clients normalized definition MD5 is % but expected % (production definition has drifted since the Phase 5B-2H preflight)', v_func2_def_md5, v_func2_md5_before;
  end if;
  ...
  perform set_config('phase5b2h_apply.f2_pre_oid',   v_func2_oid::oid::text, true);
  perform set_config('phase5b2h_apply.f2_pre_owner', v_func2_owner, true);
  perform set_config('phase5b2h_apply.f2_pre_acl',   v_func2_pre_acl::text, true);

  -- ══════════════════════════════════════════════════════════
  -- 対象外2関数（sibling）：apply前の定義MD5が既知の実測値と一致すること
  -- ══════════════════════════════════════════════════════════
  ...
  if v_sibling1_def_md5 is distinct from v_sibling1_md5 then
    raise exception 'precondition failed [SIB-MD5-1]: admin_get_other_store_weight_logs normalized definition MD5 is % but expected % (out-of-scope function has drifted)', v_sibling1_def_md5, v_sibling1_md5;
  end if;
  ...
  if v_sibling2_def_md5 is distinct from v_sibling2_md5 then
    raise exception 'precondition failed [SIB-MD5-2]: admin_get_other_store_meal_logs normalized definition MD5 is % but expected % (out-of-scope function has drifted)', v_sibling2_def_md5, v_sibling2_md5;
  end if;
end $$;

-- ══════════════════════════════════════════════════════════
-- すべてのpreconditionを通過した後にのみ到達する、対象2関数のCREATE OR
-- REPLACE FUNCTION。Phase 5B-2H preflight（本番で実行しROLLBACK済み）内の
-- CREATE OR REPLACE FUNCTION文を一字一句そのまま再利用する（整形・
-- コメント変更・空白調整なし）。変更点は各関数のRETURN QUERY内、
-- customer_number出力位置にあった「c.customer_number」を「null::text」へ
-- 置き換えた1箇所だけ。
-- ══════════════════════════════════════════════════════════
...
begin
  ...
  where n.nspname = 'public' and p.proname = 'admin_get_other_store_client';
  if v_func1_count <> 1 then
    raise exception 'postcondition failed [F1-P01]: admin_get_other_store_client does not have exactly one overload after apply (found %)', v_func1_count;
  end if;

  v_func1_oid := pg_catalog.to_regprocedure('public.admin_get_other_store_client(uuid)');
  if v_func1_oid is null or v_func1_oid::oid <> v_func1_expected_oid then
    raise exception 'postcondition failed [F1-P02]: admin_get_other_store_client OID after apply is % but expected % (OID must be unchanged by CREATE OR REPLACE)', v_func1_oid::oid, v_func1_expected_oid;
  end if;
  if v_func1_oid::oid::text <> current_setting('phase5b2h_apply.f1_pre_oid') then
    raise exception 'postcondition failed [F1-P02b]: admin_get_other_store_client OID changed during apply (before=%, after=%)', current_setting('phase5b2h_apply.f1_pre_oid'), v_func1_oid::oid::text;
  end if;
  ...
  if v_func1_owner is distinct from current_setting('phase5b2h_apply.f1_pre_owner') then
    raise exception 'postcondition failed [F1-P03]: admin_get_other_store_client owner changed during apply (before=%, after=%)', current_setting('phase5b2h_apply.f1_pre_owner'), v_func1_owner;
  end if;
  if v_func1_owner is distinct from 'postgres' then
    raise exception 'postcondition failed [F1-P03b]: admin_get_other_store_client owner is not postgres after apply (found %)', v_func1_owner;
  end if;
  if v_func1_lang is distinct from 'plpgsql' then
    raise exception 'postcondition failed [F1-P04]: admin_get_other_store_client language is not plpgsql after apply (found %)', v_func1_lang;
  end if;
  if v_func1_secdef is distinct from true then
    raise exception 'postcondition failed [F1-P05]: admin_get_other_store_client is not SECURITY DEFINER after apply';
  end if;
  if v_func1_volatile is distinct from 's' then
    raise exception 'postcondition failed [F1-P06]: admin_get_other_store_client volatility is not STABLE after apply (found %)', v_func1_volatile;
  end if;
  if v_func1_strict is distinct from false then
    raise exception 'postcondition failed [F1-P07]: admin_get_other_store_client strict is not false after apply';
  end if;
  if v_func1_leakproof is distinct from false then
    raise exception 'postcondition failed [F1-P08]: admin_get_other_store_client leakproof is not false after apply';
  end if;
  if v_func1_parallel is distinct from 'u' then
    raise exception 'postcondition failed [F1-P09]: admin_get_other_store_client parallel is not UNSAFE after apply (found %)', v_func1_parallel;
  end if;
  if v_func1_proconfig is null or array_length(v_func1_proconfig, 1) <> 1 or v_func1_proconfig[1] <> 'search_path=""' then
    raise exception 'postcondition failed [F1-P10]: admin_get_other_store_client proconfig is not exactly array[search_path=""] after apply (found %)', v_func1_proconfig;
  end if;
  ...
  if v_func1_cols_name is distinct from v_expected_cols1_name or v_func1_cols_type is distinct from v_expected_cols1_type then
    raise exception 'postcondition failed [F1-P11]: admin_get_other_store_client output columns changed after apply. names=% types=%', v_func1_cols_name, v_func1_cols_type;
  end if;

  v_func1_cn_pos  := array_position(v_func1_cols_name, 'customer_number');
  v_func1_cn_type := v_func1_cols_type[v_func1_cn_pos];
  if v_func1_cn_pos is null then
    raise exception 'postcondition failed [F1-P12]: admin_get_other_store_client no longer has a customer_number output column after apply';
  end if;
  if v_func1_cn_type is distinct from 'text' then
    raise exception 'postcondition failed [F1-P13]: admin_get_other_store_client customer_number column type is not text after apply (found %)', v_func1_cn_type;
  end if;

  v_func1_def_md5 := md5(lower(regexp_replace(btrim(v_func1_def), '\s+', ' ', 'g')));
  if v_func1_def_md5 is distinct from v_func1_md5_after then
    raise exception 'postcondition failed [F1-P14]: admin_get_other_store_client normalized definition MD5 after apply is % but expected % (does not match the Phase 5B-2H preflight measurement)', v_func1_def_md5, v_func1_md5_after;
  end if;
  if v_func1_def not ilike '%null::text%' then
    raise exception 'postcondition failed [F1-P15]: admin_get_other_store_client definition does not contain null::text after apply';
  end if;
  if v_func1_def ilike '%c.customer_number%' then
    raise exception 'postcondition failed [F1-P16]: admin_get_other_store_client definition still returns c.customer_number after apply';
  end if;
  if v_func1_def not ilike '%auth.uid()%' then
    raise exception 'postcondition failed [F1-P17a]: admin_get_other_store_client definition lost auth.uid() check after apply';
  end if;
  if v_func1_def not ilike '%role%' or v_func1_def not ilike '%''admin''%' then
    raise exception 'postcondition failed [F1-P17b]: admin_get_other_store_client definition lost admin role check after apply';
  end if;
  if v_func1_def not ilike '%store_id%' then
    raise exception 'postcondition failed [F1-P17c]: admin_get_other_store_client definition lost store_id check after apply';
  end if;
  if v_func1_def not ilike '%is_super%' then
    raise exception 'postcondition failed [F1-P17d]: admin_get_other_store_client definition lost super_admin branch after apply';
  end if;
  if v_func1_def not ilike '%permission denied%' or v_func1_def not ilike '%42501%' then
    raise exception 'postcondition failed [F1-P17e]: admin_get_other_store_client definition lost permission denied / SQLSTATE 42501 after apply';
  end if;
  ...
  if not v_func1_post_auth or not v_func1_post_svc or v_func1_post_anon or v_func1_post_public then
    raise exception 'postcondition failed [F1-P18]: admin_get_other_store_client EXECUTE privileges after apply do not match the intended set (authenticated=%, service_role=%, anon=%, public_direct=%)',
  ...
  if v_func1_acl_bad_count <> 0 then
    raise exception 'postcondition failed [F1-P19a]: admin_get_other_store_client has % ACL entr(y/ies) outside {postgres,authenticated,service_role}:EXECUTE after apply', v_func1_acl_bad_count;
  end if;
  select count(*) into v_func1_acl_good_count
  from pg_catalog.pg_proc p
  cross join lateral pg_catalog.aclexplode(coalesce(p.proacl, pg_catalog.acldefault('f', p.proowner))) a
  where p.oid = v_func1_oid::oid and a.privilege_type = 'EXECUTE' and a.grantee <> 0
    and pg_catalog.pg_get_userbyid(a.grantee) in ('postgres','authenticated','service_role');
  if v_func1_acl_good_count <> 3 then
    raise exception 'postcondition failed [F1-P19b]: admin_get_other_store_client does not have exactly 3 EXECUTE ACL entries after apply (found %)', v_func1_acl_good_count;
  end if;
  ...
  cross join lateral pg_catalog.aclexplode(coalesce(p.proacl, pg_catalog.acldefault('f', p.proowner))) a
  where p.oid = v_func1_oid::oid;
  if v_func1_post_acl::text is distinct from current_setting('phase5b2h_apply.f1_pre_acl') then
    raise exception 'postcondition failed [F1-P20]: admin_get_other_store_client ACL changed during apply (before=%, after=%)', current_setting('phase5b2h_apply.f1_pre_acl'), v_func1_post_acl::text;
  end if;
  ...
  where n.nspname = 'public' and p.proname = 'admin_list_other_store_clients';
  if v_func2_count <> 1 then
    raise exception 'postcondition failed [F2-P01]: admin_list_other_store_clients does not have exactly one overload after apply (found %)', v_func2_count;
  end if;

  v_func2_oid := pg_catalog.to_regprocedure('public.admin_list_other_store_clients(uuid)');
  if v_func2_oid is null or v_func2_oid::oid <> v_func2_expected_oid then
    raise exception 'postcondition failed [F2-P02]: admin_list_other_store_clients OID after apply is % but expected % (OID must be unchanged by CREATE OR REPLACE)', v_func2_oid::oid, v_func2_expected_oid;
  end if;
  if v_func2_oid::oid::text <> current_setting('phase5b2h_apply.f2_pre_oid') then
    raise exception 'postcondition failed [F2-P02b]: admin_list_other_store_clients OID changed during apply (before=%, after=%)', current_setting('phase5b2h_apply.f2_pre_oid'), v_func2_oid::oid::text;
  end if;
  ...
  if v_func2_owner is distinct from current_setting('phase5b2h_apply.f2_pre_owner') then
    raise exception 'postcondition failed [F2-P03]: admin_list_other_store_clients owner changed during apply (before=%, after=%)', current_setting('phase5b2h_apply.f2_pre_owner'), v_func2_owner;
  end if;
  if v_func2_owner is distinct from 'postgres' then
    raise exception 'postcondition failed [F2-P03b]: admin_list_other_store_clients owner is not postgres after apply (found %)', v_func2_owner;
  end if;
  if v_func2_lang is distinct from 'plpgsql' then
    raise exception 'postcondition failed [F2-P04]: admin_list_other_store_clients language is not plpgsql after apply (found %)', v_func2_lang;
  end if;
  if v_func2_secdef is distinct from true then
    raise exception 'postcondition failed [F2-P05]: admin_list_other_store_clients is not SECURITY DEFINER after apply';
  end if;
  if v_func2_volatile is distinct from 's' then
    raise exception 'postcondition failed [F2-P06]: admin_list_other_store_clients volatility is not STABLE after apply (found %)', v_func2_volatile;
  end if;
  if v_func2_strict is distinct from false then
    raise exception 'postcondition failed [F2-P07]: admin_list_other_store_clients strict is not false after apply';
  end if;
  if v_func2_leakproof is distinct from false then
    raise exception 'postcondition failed [F2-P08]: admin_list_other_store_clients leakproof is not false after apply';
  end if;
  if v_func2_parallel is distinct from 'u' then
    raise exception 'postcondition failed [F2-P09]: admin_list_other_store_clients parallel is not UNSAFE after apply (found %)', v_func2_parallel;
  end if;
  if v_func2_proconfig is null or array_length(v_func2_proconfig, 1) <> 1 or v_func2_proconfig[1] <> 'search_path=""' then
    raise exception 'postcondition failed [F2-P10]: admin_list_other_store_clients proconfig is not exactly array[search_path=""] after apply (found %)', v_func2_proconfig;
  end if;
  ...
  if v_func2_cols_name is distinct from v_expected_cols2_name or v_func2_cols_type is distinct from v_expected_cols2_type then
    raise exception 'postcondition failed [F2-P11]: admin_list_other_store_clients output columns changed after apply. names=% types=%', v_func2_cols_name, v_func2_cols_type;
  end if;

  v_func2_cn_pos  := array_position(v_func2_cols_name, 'customer_number');
  v_func2_cn_type := v_func2_cols_type[v_func2_cn_pos];
  if v_func2_cn_pos is null then
    raise exception 'postcondition failed [F2-P12]: admin_list_other_store_clients no longer has a customer_number output column after apply';
  end if;
  if v_func2_cn_type is distinct from 'text' then
    raise exception 'postcondition failed [F2-P13]: admin_list_other_store_clients customer_number column type is not text after apply (found %)', v_func2_cn_type;
  end if;

  v_func2_def_md5 := md5(lower(regexp_replace(btrim(v_func2_def), '\s+', ' ', 'g')));
  if v_func2_def_md5 is distinct from v_func2_md5_after then
    raise exception 'postcondition failed [F2-P14]: admin_list_other_store_clients normalized definition MD5 after apply is % but expected % (does not match the Phase 5B-2H preflight measurement)', v_func2_def_md5, v_func2_md5_after;
  end if;
  if v_func2_def not ilike '%null::text%' then
    raise exception 'postcondition failed [F2-P15]: admin_list_other_store_clients definition does not contain null::text after apply';
  end if;
  if v_func2_def ilike '%c.customer_number%' then
    raise exception 'postcondition failed [F2-P16]: admin_list_other_store_clients definition still returns c.customer_number after apply';
  end if;
  if v_func2_def not ilike '%auth.uid()%' then
    raise exception 'postcondition failed [F2-P17a]: admin_list_other_store_clients definition lost auth.uid() check after apply';
  end if;
  if v_func2_def not ilike '%role%' or v_func2_def not ilike '%''admin''%' then
    raise exception 'postcondition failed [F2-P17b]: admin_list_other_store_clients definition lost admin role check after apply';
  end if;
  if v_func2_def not ilike '%store_id%' then
    raise exception 'postcondition failed [F2-P17c]: admin_list_other_store_clients definition lost store_id check after apply';
  end if;
  if v_func2_def not ilike '%is_super%' then
    raise exception 'postcondition failed [F2-P17d]: admin_list_other_store_clients definition lost super_admin branch after apply';
  end if;
  if v_func2_def not ilike '%permission denied%' or v_func2_def not ilike '%42501%' then
    raise exception 'postcondition failed [F2-P17e]: admin_list_other_store_clients definition lost permission denied / SQLSTATE 42501 after apply';
  end if;
  ...
  if not v_func2_post_auth or not v_func2_post_svc or v_func2_post_anon or v_func2_post_public then
    raise exception 'postcondition failed [F2-P18]: admin_list_other_store_clients EXECUTE privileges after apply do not match the intended set (authenticated=%, service_role=%, anon=%, public_direct=%)',
  ...
  if v_func2_acl_bad_count <> 0 then
    raise exception 'postcondition failed [F2-P19a]: admin_list_other_store_clients has % ACL entr(y/ies) outside {postgres,authenticated,service_role}:EXECUTE after apply', v_func2_acl_bad_count;
  end if;
  select count(*) into v_func2_acl_good_count
  from pg_catalog.pg_proc p
  cross join lateral pg_catalog.aclexplode(coalesce(p.proacl, pg_catalog.acldefault('f', p.proowner))) a
  where p.oid = v_func2_oid::oid and a.privilege_type = 'EXECUTE' and a.grantee <> 0
    and pg_catalog.pg_get_userbyid(a.grantee) in ('postgres','authenticated','service_role');
  if v_func2_acl_good_count <> 3 then
    raise exception 'postcondition failed [F2-P19b]: admin_list_other_store_clients does not have exactly 3 EXECUTE ACL entries after apply (found %)', v_func2_acl_good_count;
  end if;
  ...
  cross join lateral pg_catalog.aclexplode(coalesce(p.proacl, pg_catalog.acldefault('f', p.proowner))) a
  where p.oid = v_func2_oid::oid;
  if v_func2_post_acl::text is distinct from current_setting('phase5b2h_apply.f2_pre_acl') then
    raise exception 'postcondition failed [F2-P20]: admin_list_other_store_clients ACL changed during apply (before=%, after=%)', current_setting('phase5b2h_apply.f2_pre_acl'), v_func2_post_acl::text;
  end if;

  -- ══════════════════════════════════════════════════════════
  -- 対象外2関数（sibling）：apply後も定義MD5が不変であること
  -- ══════════════════════════════════════════════════════════
  v_sibling1_oid := pg_catalog.to_regprocedure('public.admin_get_other_store_weight_logs(uuid)');
  if v_sibling1_oid is null then
    raise exception 'postcondition failed [SIB-P01]: public.admin_get_other_store_weight_logs(uuid) could not be resolved after apply';
  end if;
  v_sibling1_def_md5 := md5(lower(regexp_replace(btrim(pg_catalog.pg_get_functiondef(v_sibling1_oid::oid)), '\s+', ' ', 'g')));
  if v_sibling1_def_md5 is distinct from v_sibling1_md5 then
    raise exception 'postcondition failed [SIB-P02]: admin_get_other_store_weight_logs definition changed during apply (expected MD5 %, found %)', v_sibling1_md5, v_sibling1_def_md5;
  end if;

  v_sibling2_oid := pg_catalog.to_regprocedure('public.admin_get_other_store_meal_logs(uuid)');
  if v_sibling2_oid is null then
    raise exception 'postcondition failed [SIB-P03]: public.admin_get_other_store_meal_logs(uuid) could not be resolved after apply';
  end if;
  v_sibling2_def_md5 := md5(lower(regexp_replace(btrim(pg_catalog.pg_get_functiondef(v_sibling2_oid::oid)), '\s+', ' ', 'g')));
  if v_sibling2_def_md5 is distinct from v_sibling2_md5 then
    raise exception 'postcondition failed [SIB-P04]: admin_get_other_store_meal_logs definition changed during apply (expected MD5 %, found %)', v_sibling2_md5, v_sibling2_def_md5;
  end if;
end $$;
